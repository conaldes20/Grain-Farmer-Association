...
function init() {  
	...
	websocket.onmessage = function(evt) { 
            ...
            }else{                            //document.getElementById("incomingMsgOutput").value = msg;
                displayMessage(msg_to_client); 
            }
	};
	...
	websocket.onclose = function(evt) {
        ...
        $('#aggregationform').submit(function() {
            ...
            if (!details_array[2].includes(aggrstate)) {
                displayMessage("Aggregation point must be in " + details_array[2] + " state");
            } else if (aggrlocalgvt.includes("Local Government") || aggrstate.includes("State")) {
                displayMessage("Invalid State or Local Government selected");
            } else {                
                datastr += "aggrtown=" + aggrtown + "&aggrward=" + aggrward + "&aggrlocalgvt=" + aggrlocalgvt + "&";
        ...
        $('#farmerform').submit(function() {
            ...
            //var details_array = details.split(", ");
            /*
            if (details_array[2].includes(state)) {
                              
            } else if (localgvt.includes("Local Government") || state.includes("State")) {
                displayMessage("Invalid State or Local Government selected");
            } else {
                displayMessage("Farmer must be from " + details_array[2] + " state");
            }
            */
        ...
        $('#farmform').submit(function() {
            ...
            //var groupnum = $("#farm-group option:selected").text();
            //var farmernum = $("#farm-farmernum option:selected").text();
            ...
            var cropplanted = $("#ffarm-areaplanted").val(); 
            var areaplanted = $("#farm-areaplanted").val(); 
            var cropyear = $("#farm-year").val();
	    var cropharvested = $("#farm-harvest").val();
            ...
            /*
            if (!details_array[2].includes(state)) {
                displayMessage("Farm must be in " + details_array[2] + " state");
            } else if (localgvt.includes("Local Government") || state.includes("State")) {
                displayMessage("Invalid State or Local Government selected");
            } else {
                datastr += "longi1=" + longi1 + "&latit1=" + latit1 + "&longi2=" + longi2 + "&latit2=" + latit2 + "&";
